import { onGetBlogPosts } from '@/utils/getBlogPosts'
...
export default async function Home() {
  ...
  return (
    <main>
      ...
      <section className="md:grid-cols-3 grid-cols-1 grid gap-5 container mt-8 mb-8">
        {posts &&
          posts.map((post) => (
            <Link
              href={`/blogs/${post.id}`}
            ...
            >
              <Card className="flex flex-col gap-2 rounded-xl overflow-hidden h-full hover:bg-gray-100">
                ...
                <div className="py-1 px-10 flex flex-col gap-2">
                <p className="text-gray-500 ">
        {new Date(post.createdAt).toLocaleDateString()}
      </p> 
                  <CardTitle>{post.title}</CardTitle>
                  {parse(post.content.slice(4, 100))}...
                 <Button className="mb-5 mt-3">Read Post</Button>
                 </div>
